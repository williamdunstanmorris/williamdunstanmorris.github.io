...
* A private variable to access the ObjectVector rVector
* An accessor function to return this rVector
* Each time a sliderThatWasMoved is adjusted, an iteration over the rVector objects must be run.
* Sliders must be divided according to the four main functions in ModelProcessing. PointSourcePosition, LateReverb, DiscreteReflections and Dry/Wet.
* Each time this occurs, the rVector's objects must be updated.
*
* sliderThatWasMoved must execute -> &modelProcessing.setObjectVector().
* There is a dependency on every source, so
* Each track may have a pointsource, so it is important to have an array of PointSourceWithReverb for every track

* An example:

Onsetdelay sliderThatWasMoved ->

if(sliderThatWasMoved == onsetDelaySlider)
{
  onSetDelay = reverbProcessor.setObjectVector (getObjectVector.)
}  
